...
function isValidatedPickupAddress(value: unknown): value is PickupAddressSnapshot {
  ...
  return !!(
    (pickup.isCourierReady === true ||
      (pickup as PickupAddressSnapshot & { isAddressReady?: boolean }).isAddressReady === true) &&
    (pickup.validationLevel === "google_validated" ||
      pickup.validationLevel === "google_geo_confirmed") &&
    (pickup.sellerConfirmed === true ||
      (pickup as PickupAddressSnapshot & { userConfirmed?: boolean }).userConfirmed === true) &&
...
export async function POST(request: NextRequest) {
  ...
  if (listingIds.length !== parsed.data.listingIds.length) {
    return NextResponse.json(
      { error: "Duplicate listing IDs are not allowed." },
      { status: 400 },
    );
  }
  ...
  for (const selection of couponSelections) {
    if (couponSelectionBySeller.has(selection.sellerUid)) {
      return NextResponse.json(
        { error: "Only one coupon can be applied per seller group." },
        { status: 400 },
      );
    }
    if (seenCouponIds.has(selection.couponId)) {
      return NextResponse.json(
        { error: "The same coupon cannot be used across multiple seller groups." },
        { status: 400 },
      );
    }
  ...
  if (missingListingIndex !== -1) {
    return NextResponse.json(
      { error: `Listing not found: ${listingIds[missingListingIndex]}` },
      { status: 404 },
    );
  }
  ...
  for (const listing of listings) {
    if (listing.status !== "approved") {
      return NextResponse.json(
        { error: `"${listing.title}" is not available for protected delivery.` },
        { status: 409 },
      );
    }
  ...
  for (const sellerUid of couponSelectionBySeller.keys()) {
    if (!groupedBySeller.has(sellerUid)) {
      return NextResponse.json(
        { error: "Coupon selection does not match the current seller groups." },
        { status: 400 },
      );
    }
  }
  ...
  for (const couponId of seenCouponIds) {
    ...
    if (!coupon) {
      return NextResponse.json({ error: "Selected coupon was not found." }, { status: 404 });
    }
    if (coupon.status !== "unused") {
      return NextResponse.json(
        { error: "Selected coupon is no longer available." },
        { status: 409 },
      );
    }
    if (coupon.expiresAt && new Date(coupon.expiresAt).getTime() < Date.now()) {
      return NextResponse.json({ error: "Selected coupon has expired." }, { status: 409 });
    }
  }
  ...
  for (const [sellerUid, sellerListings] of groupedBySeller.entries()) {
    if (sellerListings.length > PROTECTED_DELIVERY_MAX_ITEMS_PER_SELLER) {
      return NextResponse.json(
        {
          error: `You can buy up to ${PROTECTED_DELIVERY_MAX_ITEMS_PER_SELLER} books per seller in one protected delivery order.`,
        },
        { status: 400 },
      );
    }
    ...
    if (invalidItem) {
      return NextResponse.json(
        { error: `Invalid price found for "${invalidItem.title}".` },
        { status: 409 },
      );
    }
    ...
    if (!serviceability.available) {
      return NextResponse.json(
        {
          error: `Protected delivery is not available from ${sellerListings[0]?.sellerName || "this seller"} to your pincode right now.`,
        },
        { status: 409 },
      );
    }
