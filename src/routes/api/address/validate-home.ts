...
export const Route = createFileRoute("/api/address/validate-home")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
        ...
        if (!parsed.success) {
          return jsonError(422, parsed.error.issues[0]?.message ?? "Invalid Home Address");
        }
        ...
        if (
          ...
          return jsonError(422, "Home Address needs a real courier-ready street address.");
        }
        ...
        try {
          ...
          const isAddressReady = isGoogleValidated || isGeoConfirmed;
          const reasonCodes = [
          ...
          return jsonOk({
            ...
            message: isGoogleValidated
              ? "Home Address is Google-validated and ready for protected delivery."
              : isGeoConfirmed
                ...
                : "Google needs a more exact Home Address before protected delivery can continue.",
            googleVerdict: {
              addressComplete,
              validationGranularity,
              geocodeGranularity,
