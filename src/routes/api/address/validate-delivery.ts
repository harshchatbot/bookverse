...
export const Route = createFileRoute("/api/address/validate-delivery")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        try {
        ...
        if (!parsed.success) {
          return jsonError(422, parsed.error.issues[0]?.message ?? "Invalid delivery address");
        }
        ...
        if (
          ...
          return jsonError(422, "Delivery address needs a real courier-ready street address.");
        }
