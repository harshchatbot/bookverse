...
function OrdersTab() {
  ...
  const { data = [], isLoading } = useQuery({ queryKey: ["admin-orders"], queryFn: () => getAllOrders() });
  ...
  async function run(orderId: string, action: "retry" | "in_transit" | "delivered") {
    ...
    try {
      if (action === "retry") {
        await apiFetch("/api/admin/shipment-retry", { method: "POST", body: JSON.stringify({ orderId }) });
      } else {
        ...
        <div key={o.id} className="rounded-2xl border border-border bg-card p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <Link to="/order/$id" params={{ id: o.id }} className="truncate font-semibold hover:underline">
                {o.listing.title}
              </Link>
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs">
            {(o.status === "paid" || !o.shipmentId) && (
              <button onClick={() => run(o.id, "retry")} disabled={busy === o.id + "retry"} className="rounded-full border border-border px-3 py-1.5 hover:bg-secondary">
                {busy === o.id + "retry" ? <Loader2 className="h-3 w-3 animate-spin" /> : "Retry shipment"}
              </button>
            )}
            <button onClick={() => run(o.id, "in_transit")} className="rounded-full border border-border px-3 py-1.5 hover:bg-secondary">
              Mark in transit
            </button>
            <button onClick={() => run(o.id, "delivered")} className="rounded-full bg-success px-3 py-1.5 font-semibold text-success-foreground">
              Mark delivered
            </button>
          </div>
        </div>
...
function PayoutsTab() {
  const qc = useQueryClient();
  const { data = [], isLoading } = useQuery({ queryKey: ["admin-payouts"], queryFn: () => getAllPayouts() });
  const [busy, setBusy] = useState<string | null>(null);
  ...
  async function markPaid(id: string) {
    ...
    setBusy(id);
    try {
      await apiFetch("/api/admin/payout-paid", { method: "POST", body: JSON.stringify({ payoutId: id, mode, reference }) });
    ...
    <div className="space-y-3">
      {data.map((p) => (
        <div key={p.id} className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-card p-4">
          <div>
            <div className="font-semibold">₹{p.amount.toLocaleString("en-IN")} → {p.sellerEmail}</div>
            <div className="text-xs text-muted-foreground">
              Order #{p.orderId.slice(-8)} · {p.status}
              {p.eligibleAt && ` · eligible ${new Date(p.eligibleAt).toLocaleString()}`}
              {p.paidAt && ` · paid ${new Date(p.paidAt).toLocaleString()} via ${p.mode} (${p.reference})`}
            </div>
          </div>
          {p.status !== "paid" && (
            <button onClick={() => markPaid(p.id)} disabled={busy === p.id} className="rounded-full bg-foreground px-4 py-2 text-xs font-semibold text-background disabled:opacity-60">
...
function DisputesTab() {
  const qc = useQueryClient();
  const { data = [], isLoading } = useQuery({ queryKey: ["admin-disputes"], queryFn: () => getAllDisputes() });

  async function resolve(id: string, status: "resolved" | "refunded") {
    const note = prompt(`Note for ${status}?`) ?? "";
    try {
      await apiFetch("/api/admin/dispute-resolve", { method: "POST", body: JSON.stringify({ disputeId: id, status, note }) });
      if (status === "refunded") {
        const dispute = data.find((d) => d.id === id);
        if (dispute) {
          await apiFetch("/api/admin/refund", { method: "POST", body: JSON.stringify({ orderId: dispute.orderId, reason: note }) });
        }
      }
    ...
    <div className="space-y-3">
      {data.map((d) => (
        <div key={d.id} className="rounded-2xl border border-border bg-card p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="font-semibold">Order #{d.orderId.slice(-8)} · {d.status}</div>
              <div className="text-xs text-muted-foreground">From {d.raisedByEmail}</div>
              <p className="mt-2 text-sm">{d.reason}</p>
              {d.resolutionNote && <p className="mt-2 text-xs italic text-muted-foreground">Note: {d.resolutionNote}</p>}
            </div>
            {d.status === "open" && (
              <div className="flex gap-2">
                <button onClick={() => resolve(d.id, "resolved")} className="rounded-full border border-border px-3 py-1.5 text-xs hover:bg-secondary">
                  Mark resolved
                </button>
                <button onClick={() => resolve(d.id, "refunded")} className="rounded-full bg-destructive px-3 py-1.5 text-xs font-semibold text-destructive-foreground">
                  Refund
                </button>
              </div>
            )}
